import React, { useState } from "react";
import Hero from "./Hero";
import { IoMdStarOutline } from "react-icons/io";
import { FaBars, FaPhoneVolume } from "react-icons/fa6";
import { FaCircle } from "react-icons/fa6";
import { Link } from "react-router-dom";
import { FaTimes } from "react-icons/fa";

const Header = () => {
  const [showMenu, setShowMenu] = useState(false);
  const [aboutDropdown, setAboutDropdown] = useState(false);
  const [servicesDropdown, setServicesDropdown] = useState(false);

  const handleMenu = () => {
    setShowMenu(!showMenu);
  };

  const toggleAboutDropdown = () => {
    setAboutDropdown(!aboutDropdown);
    setServicesDropdown(false);
  };

  const toggleServicesDropdown = () => {
    setServicesDropdown(!servicesDropdown);
    setAboutDropdown(false);
  };
  return (
    // <div className="relative overflow-hidden">
    //   {/* <div className="lg:flex hidden tracking-widest text-gray-800 items-center justify-between gap-10"> */}
    //   <div className="lg:flex flex-col md:flex-row space-y-3 items-center justify-evenly py-3 px-4 gap-10">
    //     <div className="flex items-center gap-6">
    //       <p className="uppercase flex items-center gap-3 whitespace-nowrap">
    //         <FaPhoneVolume className="text-[#e3c1a0]" />
    //         <span>+2547657382</span>
    //       </p>
    //       <button className="bg-[#e3c1a0] px-3 py-2 whitespace-nowrap rounded-md hover:bg-[#095749] hover:text-white">
    //         Book a Consultation
    //       </button>
    //     </div>
    //     <div className="flex justify-center">
    //       <a
    //         href="#!"
    //         className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
    //       >
    //         <svg
    //           xmlns="http://www.w3.org/2000/svg"
    //           fill="currentColor"
    //           viewBox="0 0 320 512"
    //           className="w-5 h-5"
    //         >
    //           <path d="M80 299.3V512H196V299.3h86.5l18-97.8H196V166.9c0-51.7 20.3-71.5 72.7-71.5c16.3 0 29.4 .4 37 1.2V7.9C291.4 4 256.4 0 236.2 0C129.3 0 80 50.5 80 159.4v42.1H14v97.8H80z" />
    //         </svg>
    //       </a>
    //       <a
    //         href="#!"
    //         className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
    //       >
    //         <svg
    //           xmlns="http://www.w3.org/2000/svg"
    //           fill="currentColor"
    //           viewBox="0 0 512 512"
    //           className="w-5 h-5"
    //         >
    //           <path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z" />
    //         </svg>
    //       </a>
    //       <a
    //         href="#!"
    //         className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
    //       >
    //         <svg
    //           xmlns="http://www.w3.org/2000/svg"
    //           fill="currentColor"
    //           viewBox="0 0 488 512"
    //           className="w-5 h-5"
    //         >
    //           <path d="M488 261.8C488 403.3 391.1 504 248 504 110.8 504 0 393.2 0 256S110.8 8 248 8c66.8 0 123 24.5 166.3 64.9l-67.5 64.9C258.5 52.6 94.3 116.6 94.3 256c0 86.5 69.1 156.6 153.7 156.6 98.2 0 135-70.4 140.8-106.9H248v-85.3h236.1c2.3 12.7 3.9 24.9 3.9 41.4z" />
    //         </svg>
    //       </a>
    //       <a
    //         href="#!"
    //         className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
    //       >
    //         <svg
    //           xmlns="http://www.w3.org/2000/svg"
    //           fill="currentColor"
    //           viewBox="0 0 448 512"
    //           className="w-5 h-5"
    //         >
    //           <path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z" />
    //         </svg>
    //       </a>
    //       <a
    //         href="#!"
    //         className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
    //       >
    //         <svg
    //           xmlns="http://www.w3.org/2000/svg"
    //           fill="currentColor"
    //           viewBox="0 0 448 512"
    //           className="w-5 h-5"
    //         >
    //           <path d="M100.3 448H7.4V148.9h92.9zM53.8 108.1C24.1 108.1 0 83.5 0 53.8a53.8 53.8 0 0 1 107.6 0c0 29.7-24.1 54.3-53.8 54.3zM447.9 448h-92.7V302.4c0-34.7-.7-79.2-48.3-79.2-48.3 0-55.7 37.7-55.7 76.7V448h-92.8V148.9h89.1v40.8h1.3c12.4-23.5 42.7-48.3 87.9-48.3 94 0 111.3 61.9 111.3 142.3V448z" />
    //         </svg>
    //       </a>
    //       <a
    //         href="#!"
    //         className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
    //       >
    //         <svg
    //           xmlns="http://www.w3.org/2000/svg"
    //           fill="currentColor"
    //           viewBox="0 0 496 512"
    //           className="w-5 h-5"
    //         >
    //           <path d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3 .3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5 .3-6.2 2.3zm44.2-1.7c-2.9 .7-4.9 2.6-4.6 4.9 .3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3 .7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3 .3 2.9 2.3 3.9 1.6 1 3.6 .7 4.3-.7 .7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3 .7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3 .7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z" />
    //         </svg>
    //       </a>
    //     </div>
    //   </div>
    //   <header className="bg-[#095749] h-20 sm:h-24 flex items-center z-30 w-full">
    //     <div className="max-w-6xl mx-auto flex items-center justify-between">
    //       <div className="flex items-center  justify-between uppercase text-teal-800 dark:text-[#e3c1a0] font-black text-3xl">
    //         <a href="/">Entrust Events</a>
    //         <button onClick={handleMenu} className="text-xl lg:hidden">
    //           {showMenu ? <FaTimes /> : <FaBars />}
    //         </button>
    //       </div>
    //       <nav className="relative border-gray-200 flex items-center justify-between gap-3">
    //         <a href="/about" className="py-2 px-6 flex">
    //           Home
    //         </a>
    //         <IoMdStarOutline className="text-[#44786d] hover:text-white" />
    //         <button
    //           id="dropdownNavbarLink"
    //           data-dropdown-toggle="dropdownNavbar"
    //           className="hover:bg-gray-50 border-b border-gray-100 md:hover:bg-transparent md:border-0 pl-3 pr-4 py-2 md:p-0 font-medium flex items-center justify-between w-full md:w-auto px-6 gap-0 uppercase"
    //           onClick={toggleAboutDropdown}
    //         >
    //           About
    //           <svg
    //             className="w-4 h-4 ml-1"
    //             fill="currentColor"
    //             viewBox="0 0 20 20"
    //             xmlns="http://www.w3.org/2000/svg"
    //           >
    //             <path
    //               fillRule="evenodd"
    //               d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
    //               clipRule="evenodd"
    //             ></path>
    //           </svg>
    //         </button>
    //         {aboutDropdown && (
    //           <div className="absolute top-4 left-[10%] bg-white text-base z-10 list-none divide-y divide-gray-100 rounded shadow my-4 w-36">
    //             <ul
    //               className="py-1"
    //               aria-labelledby="dropdownLargeButton"
    //               className=""
    //             >
    //               <li>
    //                 <a
    //                   href="#"
    //                   className="text-sm hover:bg-gray-100 text-gray-700 block px-4 py-2"
    //                 >
    //                   Rentals
    //                 </a>
    //               </li>
    //               <li>
    //                 <a
    //                   href="#"
    //                   className="text-sm hover:bg-gray-100 text-gray-700 block px-4 py-2"
    //                 >
    //                   Venue Partners
    //                 </a>
    //               </li>
    //             </ul>
    //           </div>
    //         )}
    //         <IoMdStarOutline className="text-[#44786d] hover:text-white" />
    //         <button
    //           id="dropdownNavbarLink"
    //           data-dropdown-toggle="dropdownNavbar"
    //           className="hover:bg-gray-50 border-b border-gray-100 md:hover:bg-transparent md:border-0 pl-3 pr-4 py-2 md:p-0 font-medium flex items-center justify-between w-full md:w-auto px-6 gap-0 uppercase"
    //           onClick={toggleServicesDropdown}
    //         >
    //           Services
    //           <svg
    //             className="w-4 h-4 ml-1"
    //             fill="currentColor"
    //             viewBox="0 0 20 20"
    //             xmlns="http://www.w3.org/2000/svg"
    //           >
    //             <path
    //               fillRule="evenodd"
    //               d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
    //               clipRule="evenodd"
    //             ></path>
    //           </svg>
    //         </button>
    //         {servicesDropdown && (
    //           <div className="absolute top-4 left-[24%] bg-white text-base z-10 list-none divide-y divide-gray-100 rounded shadow my-4 w-36">
    //             <ul
    //               className="py-1"
    //               aria-labelledby="dropdownLargeButton"
    //               className=""
    //             >
    //               <li>
    //                 <a
    //                   href="#"
    //                   className="text-sm hover:bg-gray-100 text-gray-700 block px-4 py-2"
    //                 >
    //                   Rentals
    //                 </a>
    //               </li>
    //               <li>
    //                 <a
    //                   href="#"
    //                   className="text-sm hover:bg-gray-100 text-gray-700 block px-4 py-2"
    //                 >
    //                   Venue Partners
    //                 </a>
    //               </li>
    //             </ul>
    //           </div>
    //         )}
    //         <IoMdStarOutline className="text-[#44786d] hover:text-white" />
    //         <a href="/gallery" className="py-2 px-6 flex">
    //           Gallery
    //         </a>
    //         <IoMdStarOutline className="text-[#44786d] hover:text-white" />
    //         <a href="/testimonials" className="py-2 px-6 flex">
    //           Testimonials
    //         </a>
    //         <IoMdStarOutline className="text-[#44786d] hover:text-white" />
    //         <a href="/blog" className="py-2 px-6 flex">
    //           Blog
    //         </a>
    //         <IoMdStarOutline className="text-[#44786d] hover:text-white" />
    //         <a href="/contacts" className="py-2 px-6 flex">
    //           Contacts
    //         </a>
    //         <button
    //           type="button"
    //           className="lg:hidden p-2 flex flex-col ml-16 cursor-pointer text-white"
    //           aria-label="Toggle navigation"
    //           onClick={handleMenu}
    //         >
    //           {showMenu ? <FaTimes size={24} /> : <FaBars size={24} />}
    //         </button>
    //         {showMenu && (
    //           <div className="absolute top-[1.95%] left-0 w-[300px] h-fit md:top-[199px] text-white bg-black opacity-100 shadow-lg z-50 lg:hidden">
    //             <ul className="flex flex-col items-center space-y-4 p-4">
    //               <li className="border-b w-full">
    //                 <Link to="/about" className="py-1 px-6 flex">
    //                   About
    //                 </Link>
    //               </li>
    //               <li className="border-b w-full">
    //                 <Link to="/events" className="py-1 px-6 flex">
    //                   Services
    //                 </Link>
    //               </li>
    //               <li className="border-b w-full">
    //                 <Link to="/rentals" className="py-1 px-6 flex">
    //                   Rentals
    //                 </Link>
    //               </li>
    //               <li className="border-b w-full">
    //                 <Link to="/gallery" className="py-1 px-6 flex">
    //                   Gallery
    //                 </Link>
    //               </li>
    //               <li className="border-b w-full">
    //                 <Link
    //                   to="/partners"
    //                   className="py-1 px-6 flex whitespace-nowrap"
    //                 >
    //                   Venue Partners
    //                 </Link>
    //               </li>
    //               <li className="border-b w-full">
    //                 <Link to="/testimonials" className="py-1 px-6 flex">
    //                   Testimonials
    //                 </Link>
    //               </li>
    //               <li className="border-b w-full">
    //                 <Link to="/blog" className="py-1 px-6 flex">
    //                   Blog
    //                 </Link>
    //               </li>
    //             </ul>
    //           </div>
    //         )}
    //         {/* </div> */}
    //       </nav>
    //     </div>
    //   </header>
    //   {/* </div> */}

    //   {/* <Hero isHeroBlurred={isHeroBlurred} /> */}
    // </div>
    <div>
      <div className="lg:flex flex-col md:flex-row space-y-3 items-center justify-evenly py-3 px-4 gap-10">
        <div className="flex items-center gap-6">
          <p className="uppercase flex items-center gap-3 whitespace-nowrap">
            <FaPhoneVolume className="text-[#e3c1a0]" />
            <span>+2547657382</span>
          </p>
          <button className="bg-[#e3c1a0] px-3 py-2 whitespace-nowrap rounded-md hover:bg-[#095749] hover:text-white">
            Book a Consultation
          </button>
        </div>
        <div className="flex justify-center">
          <a
            href="#!"
            className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 320 512"
              className="w-5 h-5"
            >
              <path d="M80 299.3V512H196V299.3h86.5l18-97.8H196V166.9c0-51.7 20.3-71.5 72.7-71.5c16.3 0 29.4 .4 37 1.2V7.9C291.4 4 256.4 0 236.2 0C129.3 0 80 50.5 80 159.4v42.1H14v97.8H80z" />
            </svg>
          </a>
          <a
            href="#!"
            className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 512 512"
              className="w-5 h-5"
            >
              <path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z" />
            </svg>
          </a>
          <a
            href="#!"
            className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 488 512"
              className="w-5 h-5"
            >
              <path d="M488 261.8C488 403.3 391.1 504 248 504 110.8 504 0 393.2 0 256S110.8 8 248 8c66.8 0 123 24.5 166.3 64.9l-67.5 64.9C258.5 52.6 94.3 116.6 94.3 256c0 86.5 69.1 156.6 153.7 156.6 98.2 0 135-70.4 140.8-106.9H248v-85.3h236.1c2.3 12.7 3.9 24.9 3.9 41.4z" />
            </svg>
          </a>
          <a
            href="#!"
            className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 448 512"
              className="w-5 h-5"
            >
              <path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z" />
            </svg>
          </a>
          <a
            href="#!"
            className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 448 512"
              className="w-5 h-5"
            >
              <path d="M100.3 448H7.4V148.9h92.9zM53.8 108.1C24.1 108.1 0 83.5 0 53.8a53.8 53.8 0 0 1 107.6 0c0 29.7-24.1 54.3-53.8 54.3zM447.9 448h-92.7V302.4c0-34.7-.7-79.2-48.3-79.2-48.3 0-55.7 37.7-55.7 76.7V448h-92.8V148.9h89.1v40.8h1.3c12.4-23.5 42.7-48.3 87.9-48.3 94 0 111.3 61.9 111.3 142.3V448z" />
            </svg>
          </a>
          <a
            href="#!"
            className="me-6 hover:bg-[#095749] hover:text-white rounded-full p-1"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 496 512"
              className="w-5 h-5"
            >
              <path d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3 .3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5 .3-6.2 2.3zm44.2-1.7c-2.9 .7-4.9 2.6-4.6 4.9 .3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3 .7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3 .3 2.9 2.3 3.9 1.6 1 3.6 .7 4.3-.7 .7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3 .7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3 .7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z" />
            </svg>
          </a>
        </div>
      </div>
      <div className="relative w-full text-white bg-[#095749]">
        <div className="flex flex-col max-w-screen-xl px-4 mx-auto md:items-center md:justify-between md:flex-row md:px-6 lg:px-8">
          <div className="p-4 flex flex-row items-center justify-between">
            <Link
              to="/"
              className="uppercase text-teal-800 dark:text-[#e3c1a0] font-black text-3xl"
            >
              Entrust Initiative
            </Link>
            <button
              onClick={handleMenu}
              className="md:hidden rounded-lg focus:outline-none focus:shadow-outline"
            >
              {showMenu ? <FaTimes size={24} /> : <FaBars size={24} />}
            </button>
          </div>
          <nav className="flex-col items-center gap-4 flex-grow pb-4 md:pb-0 inline-block md:flex md:justify-end md:flex-row">
            {/* <nav className="items-center gap-4 flex-grow pb-4 text-white uppercase md:pb-0 md:flex md:justify-end md:flex-row"> */}
            {/* <nav> */}
            <a
              // className="px-4 py-2 mt-2 text-sm font-semibold text-gray-900 bg-gray-200 rounded-lg dark-mode:bg-gray-700 dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
              className="px-4 py-2"
              href="#"
            >
              Home
            </a>
            <IoMdStarOutline className="text-[#44786d] hover:text-white" />
            <div className="relative">
              <button
                onClick={toggleAboutDropdown}
                // className="flex flex-row items-center w-full px-4 py-2 mt-2 text-sm font-semibold text-left bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:focus:bg-gray-600 dark-mode:hover:bg-gray-600 md:w-auto md:inline md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                className="whitespace-nowrap"
              >
                <span className="uppercase">About</span>
                <svg
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  className="inline w-4 h-4 mt-1 ml-1 transition-transform duration-200 transform md:-mt-1"
                >
                  <path
                    fillRule="evenodd"
                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                    clipRule="evenodd"
                  ></path>
                </svg>
              </button>
              {aboutDropdown && (
                <div className="absolute right-0 w-full mt-2 origin-top-left rounded-md z-10 shadow-lg md:w-48">
                  <div className="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-800">
                    <a
                      className="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                      href="#"
                    >
                      Link #1
                    </a>
                    <a
                      className="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                      href="#"
                    >
                      Link #2
                    </a>
                    <a
                      className="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                      href="#"
                    >
                      Link #3
                    </a>
                  </div>
                </div>
              )}
            </div>
            <IoMdStarOutline className="text-[#44786d] hover:text-white" />
            <div className="relative">
              <button
                onClick={toggleServicesDropdown}
                className="whitespace-nowrap"
                // className="flex flex-row items-center w-full px-4 py-2 mt-2 text-sm font-semibold text-left bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:focus:bg-gray-600 dark-mode:hover:bg-gray-600 md:w-auto md:inline md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
              >
                <span className="uppercase">Services</span>
                <svg
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  className="inline w-4 h-4 mt-1 ml-1 transition-transform duration-200 transform md:-mt-1"
                >
                  <path
                    fillRule="evenodd"
                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                    clipRule="evenodd"
                  ></path>
                </svg>
              </button>
              {servicesDropdown && (
                <div className="absolute right-0 w-full mt-2 origin-top-right rounded-md z-10 shadow-lg md:w-48">
                  <div className="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-800">
                    <a
                      className="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                      href="#"
                    >
                      Link #1
                    </a>
                    <a
                      className="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                      href="#"
                    >
                      Link #2
                    </a>
                    <a
                      className="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                      href="#"
                    >
                      Link #3
                    </a>
                  </div>
                </div>
              )}
            </div>
            <IoMdStarOutline className="text-[#44786d] hover:text-white" />
            <a
              // className="px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
              className="px-4 py-2"
              href="#"
            >
              Portfolio
            </a>
            <IoMdStarOutline className="text-[#44786d] hover:text-white" />
            <a
              // className="px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
              className="px-4 py-2"
              href="#"
            >
              About
            </a>
            <IoMdStarOutline className="text-[#44786d] hover:text-white" />
            <a
              // className="px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
              className="px-4 py-2"
              href="#"
            >
              Contact
            </a>
            {/* {showMenu && (
              <div className="">
                <ul className="flex flex-col items-center space-y-4 p-4">
                  <li className="border-b w-full">
                    <Link to="/about" className="py-1 px-6 flex">
                      About
                    </Link>
                  </li>
                  <li className="border-b w-full">
                    <Link to="/events" className="py-1 px-6 flex">
                      Services
                    </Link>
                  </li>
                  <li className="border-b w-full">
                    <Link to="/rentals" className="py-1 px-6 flex">
                      Rentals
                    </Link>
                  </li>
                  <li className="border-b w-full">
                    <Link to="/gallery" className="py-1 px-6 flex">
                      Gallery
                    </Link>
                  </li>
                  <li className="border-b w-full">
                    <Link
                      to="/partners"
                      className="py-1 px-6 flex whitespace-nowrap"
                    >
                      Venue Partners
                    </Link>
                  </li>
                  <li className="border-b w-full">
                    <Link to="/testimonials" className="py-1 px-6 flex">
                      Testimonials
                    </Link>
                  </li>
                  <li className="border-b w-full">
                    <Link to="/blog" className="py-1 px-6 flex">
                      Blog
                    </Link>
                  </li>
                </ul>
              </div>
            )} */}
          </nav>
        </div>
      </div>
    </div>
  );
};

export default Header;
// import React, { useState } from "react";
// import { FaBars, FaPhoneVolume } from "react-icons/fa6";
// import { FaTimes } from "react-icons/fa";

// const Header = () => {
//   const [showMenu, setShowMenu] = useState(false);
//   const [aboutDropdown, setAboutDropdown] = useState(false);
//   const [servicesDropdown, setServicesDropdown] = useState(false);

//   const handleMenu = () => {
//     setShowMenu(!showMenu);
//   };

//   const toggleAboutDropdown = () => {
//     setAboutDropdown(!aboutDropdown);
//     setServicesDropdown(false);
//   };

//   const toggleServicesDropdown = () => {
//     setServicesDropdown(!servicesDropdown);
//     setAboutDropdown(false);
//   };

//   return (
//     <div className="relative overflow-hidden">
//       <div className="lg:flex flex-col md:flex-row space-y-3 items-center justify-evenly py-3 px-4 gap-10">
//         <div className="flex items-center gap-6">
//           <p className="uppercase flex items-center gap-3 whitespace-nowrap">
//             <FaPhoneVolume className="text-[#e3c1a0]" />
//             <span>+2547657382</span>
//           </p>
//           <button className="bg-[#e3c1a0] px-3 py-2 whitespace-nowrap rounded-md hover:bg-[#095749] hover:text-white">
//             Book a Consultation
//           </button>
//         </div>
//         <div className="flex justify-center">
//           {/* Add your social icons here */}
//         </div>
//         <div className="flex items-center gap-4">
//           <button className="lg:hidden" onClick={handleMenu}>
//             {showMenu ? <FaTimes /> : <FaBars />}
//           </button>
//           <div className="relative">
//             <button onClick={toggleAboutDropdown} className="flex items-center">
//               About
//             </button>
//             {aboutDropdown && (
//               <div className="absolute left-0 bg-white shadow-lg z-10">
//                 <ul className="flex flex-col">
//                   <li>
//                     <Link to="/about-us" className="block px-4 py-2">
//                       About Us
//                     </Link>
//                   </li>
//                   <li>
//                     <Link to="/our-team" className="block px-4 py-2">
//                       Our Team
//                     </Link>
//                   </li>
//                 </ul>
//               </div>
//             )}
//           </div>
//           <div className="relative">
//             <button
//               onClick={toggleServicesDropdown}
//               className="flex items-center"
//             >
//               Services
//             </button>
//             {servicesDropdown && (
//               <div className="absolute left-0 bg-white shadow-lg z-10">
//                 <ul className="flex flex-col">
//                   <li>
//                     <Link to="/service1" className="block px-4 py-2">
//                       Service 1
//                     </Link>
//                   </li>
//                   <li>
//                     <Link to="/service2" className="block px-4 py-2">
//                       Service 2
//                     </Link>
//                   </li>
//                 </ul>
//               </div>
//             )}
//           </div>
//         </div>
//       </div>
//       {/* Mobile menu */}
//       {showMenu && (
//         <div className="flex flex-col md:hidden bg-white shadow-lg">
//           <div className="flex flex-col">
//             <Link to="/about-us" className="px-4 py-2">
//               About Us
//             </Link>
//             <Link to="/our-team" className="px-4 py-2">
//               Our Team
//             </Link>
//             {/* Add more mobile links as needed */}
//           </div>
//         </div>
//       )}
//     </div>
//   );
// };

// export default Header;
